...
local_ips() {
    ...
    if command -v ip >/dev/null 2>&1; then
        ip addr show 2>/dev/null | grep -oE 'inet ([0-9]{1,3}\.){3}[0-9]{1,3}' | awk '{print $2}' | grep -v '^127\.' | head -n1
        return
    fi

    # 方法 2: 使用 ifconfig（旧系统）
    if command -v ifconfig >/dev/null 2>&1; then
        ifconfig -a 2>/dev/null | grep -oE 'inet (addr:)?([0-9]{1,3}\.){3}[0-9]{1,3}' | awk '{print $2}' | grep -v '^127\.' | head -n1
        return
    fi

    # 方法 3: fallback 到 /proc/net/fib_trie（无网络工具时）
    if [ -r /proc/net/fib_trie ]; then
        cat /proc/net/fib_trie 2>/dev/null | awk '/32 host/ { getline; print $2 }' | grep -v '^127\.' | head -n1
        return
    fi

    # 终极 fallback：返回空或 127.0.0.1
    echo "127.0.0.1"
}
